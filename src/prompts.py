...
When complete panel and workstation records agree on both the required workstation and supported operation, report the verified match. Do not escalate a compatible scan merely because an optional or overly broad SOP search returned no match; refine the search using the verified operation or an exact SOP source ID. Escalation remains required for genuinely missing production facts, unknown panels, physical-label conflicts, inconsistent records, and unsupported operational parameters.
