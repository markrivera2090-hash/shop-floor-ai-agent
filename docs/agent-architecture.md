...
A deterministic response gate blocks ungrounded production guidance, numeric answers to unsupported machine-parameter questions, permission to proceed at a wrong workstation, invented unknown-panel details, and invented claims of personal supervisor contact. The model first classifies whether a request belongs to the shop-floor service. Clearly unrelated requests are answered with the supported scope and cannot execute tools or create escalation history. For relevant requests, unavailable or inconsistent grounding triggers the escalation tool; a deterministic safety backstop records the escalation if the model attempts to answer without it. Workstation compatibility is determined from successful structured panel and workstation results by comparing both the required workstation ID and operation. A wrong workstation with a grounded next step does not escalate merely because it is a mismatch. Conversely, complete matching records produce a deterministic compatibility response and cannot be escalated solely because an optional or overly broad SOP search missed.

SOP retrieval resolves an exact source ID such as `SOP-GENERAL-001` directly, gives strong weight to explicit intent aliases and title terms, removes generic verification words, requires a minimum relevance score, and retains only closely scored secondary sections. Edge Banding and Drilling are mutually exclusive search intents, while mismatch, unsupported-parameter, and escalation sections require matching issue language.
