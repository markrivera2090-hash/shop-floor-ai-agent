...
A deterministic response gate blocks ungrounded production guidance, numeric answers to unsupported machine-parameter questions, permission to proceed at a wrong workstation, invented unknown-panel details, and claims that simulated escalation contacted a real person. Workstation compatibility is determined from successful structured panel and workstation results by comparing both the required workstation ID and operation; an incidental mismatch SOP search result is not treated as proof of a mismatch. A directly reported physical-label discrepancy is a mandatory stop-and-escalate condition. The model is instructed to choose the escalation tool, and a narrow deterministic safety backstop invokes it if the model attempts to answer that scenario without doing so. This backstop applies only to the explicit mismatch safety case and does not impose one fixed tool sequence on other requests.
